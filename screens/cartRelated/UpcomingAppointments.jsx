...
const UpcomingAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.approved_pathology_appointments);
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.headerRow}>
        ...
        />
        <Text style={styles.unitName}>{item.dialysis?.dialysisUnitName || 'Lab Appointment'}</Text>
      </View>
      ...
      <View style={styles.infoRow}>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.value}>{item.payment_method || 'N/A'}</Text>
        </View>
      </View>
      ...
      <View style={styles.actions}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('GetMoreDetail', { id: item._id })}
        >
          <MaterialCommunityIcons name="information-outline" size={18} color="#fff" />
          <Text style={styles.btnText}>Get More Detail</Text>
        </TouchableOpacity>
