...
const InfectionDiseasePanels = ({ navigation }) => {
  const dispatch = useDispatch();
  const [testData, setTestData] = useState([]);
  const [cartCount, setCartCount] = useState(0);

  const fetchInfectionTests = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.comparison_infection_disease_tests);
      setTestData(response?.data || []);
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching infection tests:', error);
      dispatch(hideLoader());
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        title="Infection Disease Panels"
                ...
                <View style={styles.cardContent}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.testName}>
                      <Icon name="bug-outline" size={16} color={COLORS.primary} /> {item.testDescription?.testName}
                    </Text>
...
export default InfectionDiseasePanels;
