...
const UrineStool = ({ navigation }) => {
  ...
  const fetchUrineStoolTests = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.comparison_urine_stool_tests);
      setTestData(response?.data || []);
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching urine/stool tests:', error);
      dispatch(hideLoader());
    }
  };
