...
const ImagingDiagnostics = ({ navigation }) => {
  ...
  const fetchImagingTests = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.comparison_imaging_diagnostics_tests);
      setTestData(response?.data || []);
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching imaging diagnostics:', error);
      dispatch(hideLoader());
    }
  };
                ...
                <View style={styles.cardContent}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.testName}>
                      <Icon name="image-filter-center-focus" size={16} color={COLORS.primary} /> {item.testDescription?.testName}
                    </Text>
