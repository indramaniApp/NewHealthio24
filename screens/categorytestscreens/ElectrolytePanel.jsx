...
const ElectrolytePanel = ({ navigation }) => {
  ...
  const fetchElectrolyteTests = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.comparison_electrolyte_tests);
      setTestData(response?.data || []);
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching electrolyte tests:', error);
      dispatch(hideLoader());
    }
  };
