...
const LiverFunctionTest = ({ navigation }) => {
  const dispatch = useDispatch();
  const [testData, setTestData] = useState([]);
  const [cartCount, setCartCount] = useState(0);

  const fetchLiverTests = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(ENDPOINTS.comparison_liver_tests);
      setTestData(response?.data || []);
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching liver tests:', error);
      dispatch(hideLoader());
    }
  };
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    // Background color yahan se hata dein
    // backgroundColor: '#fff', 
