...
const CompletedReceipt = ({ navigation, route }) => {
  ...
  const fetchReceipt = async () => {
    try {
      ...
      console.log('Completed receipt fetched: ', response.data);
  ...
  const generatePDF = async () => {
    ...
    const htmlContent = `
      <html>
        ...
        <body>
          <h1>Healthio24</h1>
          <h2>Completed Receipt / Invoice</h2>
    ...
    try {
      const file = await RNHTMLtoPDF.convert({
        html: htmlContent,
        fileName: `CompletedReceipt_${receipt._id}`,
        directory: 'download',
      });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <Header title="Completed Receipt" onBackPress={() => navigation.goBack()} />

      <View style={{ flex: 1, padding: 16 }}>
        <View style={styles.card}>
          <Text style={styles.headerText}>Healthio24</Text>
          <Text style={styles.subHeader}>Completed Receipt / Invoice</Text>
