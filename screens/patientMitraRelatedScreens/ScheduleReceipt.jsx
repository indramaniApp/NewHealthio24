...
const ScheduleReceipt = ({ navigation, route }) => {
    ...
    const fetchReceipt = async () => {
        try {
            setLoading(true);
            const response = await ApiService.get(`${ENDPOINTS.debit_transaction_pm_package_book}/${id}`);
            console.log('PM Package receipt fetched: ', response.data);
    ...
    const generatePDF = async () => {
        ...
        const htmlContent = `
            <html>
              ...
              <body>
                <h1>Healthio24</h1>
                <h2>PM Package Receipt / Invoice</h2>
                ...
                <div class="row">
                  <div>${receipt.purpose || 'PM Package Booking'}</div>
        ...
        try {
            const file = await RNHTMLtoPDF.convert({
                html: htmlContent,
                fileName: `PMPackageReceipt_${receipt._id}`,
                directory: 'download',
            });
    ...
    return (
        // 2. Wrap the entire component in LinearGradient
        <LinearGradient
            colors={['#00b4db', '#fff', '#fff']}
            style={{ flex: 1 }}
        >
            <SafeAreaView style={{ flex: 1 }}>
                <Header title="PM Package Receipt" onBackPress={() => navigation.goBack()} />
                ...
                {loading || !receipt ? (
                    ...
                    <View style={{ flex: 1, padding: 16 }}>
                        <View style={styles.card}>
                            ...
                            <Text style={styles.subHeader}>PM Package Receipt / Invoice</Text>
...
const styles = StyleSheet.create({
    loaderContainer: {
        ...
        alignItems: 'center',
        // 3. Removed background color to show gradient
    },
    card: {
        padding: 12,
        borderRadius: 16,
