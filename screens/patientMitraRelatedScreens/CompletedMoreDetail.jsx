...
const CompletedMoreDetail = () => {
  ...
  const fetchDetails = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(`${ENDPOINTS.pm_package_book_detail}/${id}`);
      console.log('Completed Appointment Detail:', response);
  ...
  const renderHeader = () => (
    <View style={styles.screenHeader}>
      ...
      <Text style={styles.screenTitle}>Completed PM Package Details</Text>
    </View>
  );

  const renderCard = ({ item }) => (
    <View style={styles.card}>
      <View style={[styles.cardHeader, { backgroundColor: '#388E3C' }]}>
        <MaterialIcons name="assignment-turned-in" size={20} color="#fff" />
        <Text style={styles.cardHeaderText}>Completed Booking</Text>
      ...
      <View style={styles.section}>
        <InfoLabel label="Payment" value={`₹${item?.payment_amount} (${item?.payment_status})`} />
        <InfoLabel label="Method" value={item?.payment_method} />
        <InfoLabel label="Txn ID" value={item?.payment_transaction_id} />
      </View>

      <View style={styles.section}>
        <InfoLabel label="Notes" value={item?.notes || 'N/A'} />
      </View>

      <View style={{ marginTop: 10 }}>
        <Text style={styles.bookedOn}>Completed on: {formatDateTime(item?.completed_date)}</Text>
      </View>
    </View>
  );
...
export default CompletedMoreDetail;
