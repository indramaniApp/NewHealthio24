...
const PhysiotherapyBookByPayment = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    if (!patientName || !patientGender || !patientAge || !reasonForVisit) {
      Alert.alert('Missing Fields', 'Please fill all patient details including reason for visit');
      return;
    }
    ...
    try {
      ...
      const finalUrl = `${ENDPOINTS.book_appointment_physiotherapy}/${centreId}`;
  ...
  const handlePayment = (orderId) => {
    const options = {
      description: 'Physiotherapy Appointment Payment',
  ...
  const verifyPayment = async (payment_id, signature, order_id) => {
    try {
      ...
      const response = await ApiService.post(ENDPOINTS.payment_physiotherapy_verify, payload, true, false);
      dispatch(hideLoader());
  ...
  return (
    <View style={styles.container}>
      ...
      <Header title="Book Physiotherapy" onBackPress={() => navigation.goBack()} />
...
export default PhysiotherapyBookByPayment;
