...
const PhysiotherapRecipt = ({ navigation, route }) => {
  ...
  const fetchReceipt = async () => {
    try {
      setLoading(true);
      const response = await ApiService.get(`${ENDPOINTS.debit_transaction_physiotherapy_book}/${id}`);
      console.log('Physiotherapy receipt fetched: ', response.data);
  ...
  const generatePDF = async () => {
    ...
    const htmlContent = `
      <html>
        ...
        <body>
          <h1>Healthio24</h1>
          <h2>Physiotherapy Receipt / Invoice</h2>
          ...
          <div class="row">
            <div>${receipt.purpose || 'Physiotherapy Booking'}</div>
    ...
    try {
      const file = await RNHTMLtoPDF.convert({
        html: htmlContent,
        fileName: `PhysiotherapyReceipt_${receipt._id}`,
        directory: 'download',
      });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <Header title="Physiotherapy Receipt" onBackPress={() => navigation.goBack()} />

      <View style={{ flex: 1, padding: 16 }}>
        <View style={styles.card}>
          <Text style={styles.headerText}>Healthio24</Text>
          <Text style={styles.subHeader}>Physiotherapy Receipt / Invoice</Text>
...
export default PhysiotherapRecipt;
