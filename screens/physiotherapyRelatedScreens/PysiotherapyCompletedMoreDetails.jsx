...
const PysiotherapyCompletedMoreDetails = () => {
  ...
  const fetchCompletedDetails = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(`${ENDPOINTS.physiotherapy_book}/${id}`);
      console.log('Physiotherapy completed detail response:', response);
      if (response?.status === 'success' && Array.isArray(response.data)) {
        setData(response.data);
      }
      dispatch(hideLoader());
    } catch (error) {
      console.log('Error fetching completed details:', error);
      dispatch(hideLoader());
    }
  };
  ...
  const renderHeader = () => (
    <View style={styles.screenHeader}>
      ...
      <Text style={styles.screenTitle}>Completed Details</Text>
    </View>
  );
  ...
  const renderCard = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.cardHeaderText}>Completed Physiotherapy</Text>
  ...
  <Text style={styles.bookedOn}>Completed At: {formatDateTime(item?.completedAt || item?.updatedAt)}</Text>
</View>
...
export default PysiotherapyCompletedMoreDetails;
