...
const PhysiotherapyMoreDetails = () => {
  ...
  useEffect(() => {
    fetchPhysiotherapyDetails();
  }, []);

  const fetchPhysiotherapyDetails = async () => {
    try {
      dispatch(showLoader());
      const response = await ApiService.get(`${ENDPOINTS.physiotherapy_book}/${id}`);
      console.log('Physiotherapy detail response:==', response);
      if (response?.status === 'success' && Array.isArray(response.data)) {
        setData(response.data);
      }
      dispatch(hideLoader());
    } catch (error) {
      console.log('Physiotherapy fetch error:', error);
      dispatch(hideLoader());
    }
  };
  ...
  const renderHeader = () => (
    <View style={styles.screenHeader}>
      ...
      <Text style={styles.screenTitle}>Physiotherapy Details</Text>
    </View>
  );
  ...
  const renderCard = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <Text style={styles.cardHeaderText}>Physiotherapy Booking</Text>
...
export default PhysiotherapyMoreDetails;
