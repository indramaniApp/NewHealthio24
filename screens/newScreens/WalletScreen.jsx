...
const WalletScreen = ({ navigation }) => {
      ...
      <View style={styles.transactionLeft}>
        <View style={[styles.iconWrapper, { backgroundColor: '#EEF1F5' }]}>
          <Icon
            ...
            size={22}
            color={item.transaction_type === 'credit' ? '#0f9d58' : '#033687'}
          />
        </View>
        <View>
          <Text style={[styles.transactionTitle, { color: '#111' }]}>₹{item.amount} - {item.purpose}</Text>
          <Text style={[styles.transactionDate, { color: '#111' }]}>{item.createdAt}</Text>
          <Text style={{ fontSize: 12, color: '#111' }}>Method: {item.payment_method}</Text>
          <Text style={{ fontSize: 12, color: '#111' }}>Txn ID: {item.payment_id}</Text>
        </View>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: '#F4F6FA' }]}>
      ...
      <View style={[styles.walletCard, { backgroundColor: '#ffffff' }]}>
        <View style={styles.walletStripe} />
        <View style={styles.walletContent}>
          <Icon
            name="wallet-outline"
            size={120}
            color="rgba(3, 54, 135, 0.05)"
            style={{ position: 'absolute', right: 10, top: 10 }}
          />
          <View style={styles.brandCircle}>
            <Icon name="wallet" size={24} color="#fff" />
          </View>
          <Text style={[styles.walletLabel, { color: '#888' }]}>Wallet Balance</Text>
          <Text style={[styles.walletAmount, { color: '#000' }]}>₹{walletDetails?.amount?.toFixed(2) ?? '0.00'}</Text>
          <Text style={{ color: '#033687', fontWeight: 'bold', fontSize: 13, marginBottom: 4 }}>
            Last Added: ₹{walletDetails?.add_amount?.toFixed(2) ?? '0.00'}
          </Text>
          <TouchableOpacity style={styles.addMoneyButton} onPress={onAddAmountPress}>
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, { backgroundColor: '#FFF' }]}>
            <Text style={[styles.modalTitle, { color: '#000' }]}>Enter Amount</Text>
            <TextInput
              value={amountToAdd}
              ...
              style={[styles.input, { color: '#000', borderBottomColor: '#ccc' }]}
            />
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalCancel} onPress={() => setModalVisible(false)}>
                <Text style={{ color: '#033687', fontWeight: '600' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.modalSubmit} onPress={onConfirmAddAmount}>
...
const styles = StyleSheet.create({
  ...
  walletStripe: { width: 10, backgroundColor: '#033687' },
  walletContent: { flex: 1, padding: 24, position: 'relative' },
  walletLabel: { fontSize: 15, marginBottom: 4, fontWeight: '500' },
  walletAmount: { fontSize: 36, fontWeight: 'bold', marginBottom: 10 },
  addMoneyButton: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  modalContainer: { borderRadius: 16, padding: 20 },
